...
class PreferencesTittleDialog(gtk.Dialog):
    __gtype_name__ = "PreferencesTittleDialog"
    prefernces = {}

    def __init__(self):
        """__init__ - This function is typically not called directly.
        Creation of a PreferencesTittleDialog requires redeading the associated ui
        file and parsing the ui definition extrenally,
        and then calling PreferencesTittleDialog.finish_initializing().

        Use the convenience function NewPreferencesTittleDialog to create
    ...
    def get_preferences(self):
        ...
        preferences for tittle. Creates a couchdb record if
        necessary.
        """
...
def NewPreferencesTittleDialog():
    """NewPreferencesTittleDialog - returns a fully instantiated
    PreferencesTittleDialog object. Use this function rather than
    creating a PreferencesTittleDialog instance directly.
    """

    #look for the ui file that describes the ui
    ui_filename = os.path.join(getdatapath(), 'ui', 'PreferencesTittleDialog.ui')
    if not os.path.exists(ui_filename):
        ui_filename = None

    builder = gtk.Builder()
    builder.add_from_file(ui_filename)
    dialog = builder.get_object("preferences_tittle_dialog")
    dialog.finish_initializing(builder)
    return dialog

if __name__ == "__main__":
    dialog = NewPreferencesTittleDialog()
    dialog.show()
    gtk.main()
